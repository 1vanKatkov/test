...
from app.config import MAX_AUTH_SECRET, MAX_AUTH_SKEW_SECONDS
...
def verify_max_signature(
    ...
    now = int(time.time())
    if abs(now - ts) > MAX_AUTH_SKEW_SECONDS:
        raise HTTPException(status_code=401, detail="Auth timestamp expired")

    expected = _sign(user_id=user_id, timestamp=timestamp, nonce=nonce, body_digest=body_digest, secret=MAX_AUTH_SECRET)
...
def build_signature_for_debug(user_id: str, nonce: str, body: dict) -> dict:
    payload = json.dumps(body, separators=(",", ":"), ensure_ascii=False).encode("utf-8")
    timestamp = str(int(time.time()))
    digest = _payload_digest(payload)
    signature = _sign(user_id, timestamp, nonce, digest, MAX_AUTH_SECRET or "")
    return {
